...
with dask.config.set({"distributed.worker.resources.cores": 16}):
    cluster = LocalCluster(n_workers=8, threads_per_worker=16, processes=False)
client = Client(cluster)


#NOTE: Swapping move is enabled right now!


# DEFINE PATHS HERE
MPATH = "12-6_Nucleation/"
SHPATH_TRJCONV = MPATH + "scripts/trjconv.sh"
SHPATH_SIMULATE = MPATH + "scripts/simulate.sh"
SHPATH_EXTEND = MPATH + "scripts/extend.sh"
GROPATH = MPATH + "masters/LJ-8192.gro"
INMASTERPATH = MPATH + "masters/npt.mdp"
TOPPATH = MPATH + "masters/12-6.top"
LOG_PATH = "RETIS.log"
...
temperature = 100.4*8.31446/1000
beta = 1.0/temperature

interfaces = [ 44.5, 54.5, 69.5, 79.5,
               89.5, 99.5, 109.5, 129.5, 144.5, 
               159.5, 179.5, 194.5, 214.5, 234.5,
               269.5, 329.5, 419.5, 599.5 ]

# number of steps to run before checking for crossing
nsteps = [500, 500, 500, 500, 500, 500, 1000, 1000, 1000, 1000, 1000, 1000, 1000, 2000, 2000, 2000, 2000 ]
...
def read_paths(glmoveid):
    ...
    for interface in range(ninterfaces):
        ...
        path = np.genfromtxt(filen,comments=None,dtype=[('pathid','U50'),('time','f8'),('op','i8'),('rev','i8')],encoding='utf8')
...
def global_move_generator(glmoveid):
    ...
    swap = True if np.random.ranf() < 0.5 else False
    if swap:
        FLOG.write("Swap: ")
...
def ensemble_move_generator(interface,path,glmoveid):
    ...
    if np.random.ranf() < 1.5: # we're not doing a reversal move
        path = perform_shooting_move(interface,path,moveid,alwaysAccept=False)
    else:
...
def call_trjconv(interface,trajname,ntime,moveid,reverse,zeroswap=False):
    ...
    # The above script copies the gro file to an _old.gro file
...
def call_backward_simulation(interface,moveid,nstep):
    ...
    opfilepath = epath + "op/" + moveid + "_q6_rev.xvg"
...
def call_simulation(interface,moveid,nstep):
    ...
    opfilepath = epath + "op/" + moveid + "_q6.xvg"
...
def extend_simulation(interface,moveid,nstep,itr,backward=False):
    ...
    if backward == True:
        ...
        opfilepath = epath + "op/" + moveid + "_extend-" + str(itr) + "_q6_rev.xvg"
        ...
        opfilepath = epath + "op/" + moveid + "_extend-" + str(itr) + "_q6.xvg"
...
def extract_path(interface,moveid,itr,partial_path,max_length,backward=False):
    ...
    if backward == True:
        if partial_path is None:
            opfilepath = MPATH + str(interface) + "/op/" + moveid + "_q6_rev.xvg"
    else:
        if partial_path is None:
            opfilepath = MPATH + str(interface) + "/op/" + moveid + "_q6.xvg"
    
    # Save new path section into temporary structure
    tmp_seg = np.genfromtxt(opfilepath,comments=None,skip_header=12,dtype=[('time','f8'),('op','i8')])
...
def extract_path_zero(interface,moveid,itr,partial_path):
    ...
    if partial_path is None:
        ...
        opfilepath = MPATH + str(interface) + "/op/" + moveid + "_extend-" + str(itr) + "_q6.xvg"
...
def extract_path_minus(interface,moveid,itr,partial_path):
    ...
    if partial_path is None:
        ...
        opfilepath = MPATH + str(interface) + "/op/" + moveid + "_extend-" + str(itr) + "_q6_rev.xvg"
...
def perform_shooting_move(interface,path,moveid,alwaysAccept):
    ...
    if reject == True:
        #FLOG.write("Shoot {} {} Reject noShootingPoint\n".format(interface,moveid))
        shootlog = "Shoot {} {} Reject noShootingPoint\n".format(interface,moveid)
